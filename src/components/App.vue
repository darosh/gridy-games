<template>
  <v-app id="app" :dark="$store.state.dark">
    <v-navigation-drawer v-model="Shared.menu" :touchless="!$route.meta.home" class="pb-0" fixed left app disable-resize-watcher disable-route-watcher>
      <g-menu/>
    </v-navigation-drawer>
    <v-navigation-drawer v-model="Shared.drawer" :touchless="$route.meta.home" class="pb-3" fixed right app disable-resize-watcher disable-route-watcher>
      <!-- TODO: use route view? -->
    ...
    <v-snackbar v-model="showUpdate" :timeout="1e10" color="grey darken-3" left bottom>
      <v-flex @click="update = false">Update ready!</v-flex>
      <v-btn flat color="light-blue" @click.native="reload()">Reload</v-btn>
    </v-snackbar>
    <g-patterns />
