<template>
  <div>
    <v-toolbar dense flat color="transparent">
      ...
      <v-spacer/>
      <v-btn icon @click="switchOponents()">
        <v-icon>swap_vert</v-icon>
      </v-btn>
    </v-toolbar>
    <div class="mx-3 mb-3 ">
      <v-select v-model="player1" :items="players" :class="$store.state.dark ? 'grey darken-2' : 'grey lighten-4'" label="Player 1" solo single-line bottom hide-details class="mb-2" />
      <v-select v-model="player2" :items="players" :class="$store.state.dark ? 'grey darken-2' : 'grey lighten-4'" label="Plaayer 2" solo single-line bottom hide-details class="mb-2" />
    </div>
    <v-divider/>
    <div>
      <v-toolbar dense flat color="transparent">
      ...
      <div class="mx-3 mb-3">
        <v-layout v-if="isHuman(this.$store.state.player[1]) && isHuman(this.$store.state.player[2])" row mx-3 align-center style="margin-top: -16px">
          <v-flex class="body-1">Timer</v-flex>
          <div>
            <v-select v-model="timer" :items="timers" single-line bottom hide-details class="mb-3" />
          </div>
        </v-layout>
        ...
        <v-layout v-if="canVibrate" row mx-3 align-center>
    ...
    <div class="mx-3 mb-3">
      <v-toolbar dense flat color="transparent">
        ...
        <v-spacer/>
        <div v-if="Shared.game.moves.length" class="body-2">{{ Shared.game.moves.length }}</div>
      </v-toolbar>

      <v-text-field v-model="record" :class="$store.state.dark ? 'grey darken-2' : 'grey lighten-4'" solo spellcheck="false" name="input-turns" placeholder="Insert turns" multi-line auto-grow rows="3" />
    </div>
    <v-divider></v-divider>
    <v-toolbar dense flat color="transparent">
