<template>
  <div>
    ...
    <div class="mx-3 mb-3">
      <v-layout v-for="m in meta" :key="m.title" row mx-3 align-center mb-1>
        <v-flex class="body-2">{{ m.title }}</v-flex>
        <div v-if="m.link" class="body-1">
          <a :href="m.link" target="_blank" rel="noopener" class="light-blue--text">{{ m.value }}</a>
        </div>
    ...
    <div class="mx-3 mb-3">
      <v-layout v-for="d in dependencies" :key="d.text" row mx-3 align-center mb-1>
        <div class="body-1 pr-3">
          <a :href="d.link" target="_blank" rel="noopener" class="light-blue--text">{{ d.text }}</a>
        </div>
        <v-divider/>
