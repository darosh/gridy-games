<template>
  <div>
    ...
    <div class="mx-3 mb-3">
      <v-layout v-for="m in meta" :key="m.title" row mx-3 align-center mb-3>
        <v-flex class="body-2">{{m.title}}</v-flex>
        <div v-if="m.link" class="body-1"><a target="_blank" rel="noopener" :href="m.link" class="light-blue--text">{{m.value}}</a></div>
