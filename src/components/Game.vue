  ...
  <div v-resize="onResize" :class="theme">
    <v-layout column wrap align-center align-content-center>
      <div class="mt-1" style="text-align: center">
        <v-layout row mb-2 px-2 style="height: 48px">
          <v-flex xs6 class="pr-2">
            <player-card v-if="game" :value="1" :game="game" :waiting="robotTurn" :timer="timerValue" :counter="counter" :transition="transition" :winner="winner" />
          </v-flex>
          <div style="min-width: 32px;" class="text-xs-center">
            <div v-if="game.score" class="display-1">
          ...
          <v-flex xs6 class="pl-2">
            <player-card v-if="game" :value="2" :game="game" :waiting="robotTurn" :switcher="canSwitch" :timer="timerValue" :counter="counter" :transition="transition" :class="{clickable: canSwitch}" :winner="winner" @click.native="canSwitch && switchPlayer()" />
          </v-flex>
        </v-layout>
        <div :style="{width: size[vertical ? 1 : 0] + 'px', height: size[vertical ? 0 : 1] + 'px'}" :class="{vertical: vertical}" class="d-inline-block">
          <svg :width="size[0]" :height="size[1]" :class="theme" class="d-block">
            <defs>
              <pattern id="stripes" patternTransform="translate(0,0) scale(3,3), rotate(-45)" height="1" width="3" patternUnits="userSpaceOnUse">
                <rect height="2" width="1" y="-0.5" x="0" />
              </pattern>
              <pattern id="stripes-30" patternTransform="translate(0,0) scale(3,3), rotate(-15)" height="1" width="3" patternUnits="userSpaceOnUse">
                <rect height="2" width="1" y="-0.5" x="0" />
              </pattern>
            </defs>
            <rect :x="margin/2 + 0.5" :y="margin/2 + 0.5" :width="size[0] - margin" :height="size[1] - margin" />
            <g :transform="'translate('+ translate +')'">
              <line v-for="(l, k) in lines" :key="k" :x1="center(l[0]).x" :y1="center(l[0]).y" :x2="center(l[1]).x" :y2="center(l[1]).y" :class="l[2] ? 'line-' + l[2] : ''" />
              <g v-for="t in game.grid.tiles" :key="t.key" :transform="'translate(' + center(t) + ')'">
                <polygon :points="vertices" :class="{['angle-' + Math.abs(game.grid.orientation * game.grid.angle)] : true, clickable: t.highlighted && !robotTurn, ['value-' + (t.data || 0)]: true, possible: t.highlighted, odd: t.odd, animate: true, waiting: t.highlighted && robotTurn}" :transform="'rotate(' + (game.grid.orientation * game.grid.angle) + ')'" @click="move(t)" /> </g>
              <line v-for="(l, k) in rulers" :key="'r' + k" :x1="center(l[0]).x" :y1="center(l[0]).y" :x2="center(l[1]).x" :y2="center(l[1]).y" :class="l[2] ? 'line-' + l[2] : ''" />
              <g v-if="!isMove" v-for="t in game.grid.tiles" :key="'c' + t.key" :transform="'translate(' + center(t) + ')'">
                <transition name="scale-transition">
                  <circle v-if="t.data" :r="game.grid.radius * game.grid.scale * 0.85 - 2" :class="{clickable: t.highlighted && !robotTurn, ['token-' + t.data]: true}" cx="0" cy="0" class="animate" @click="move(t)" />
                </transition>
              </g>
              <g v-if="isMove" v-for="(t, k) in stones" :key="'s' + k" :style="{transform: 'translate(' + center(t.tile, 'px') + ')'}" class="stone">
                <!-- <transition name="scale-transition"> -->
                <circle :r="game.grid.radius * game.grid.scale * 0.85 - 2" :class="{clickable: t.tile.highlighted && !robotTurn, ['token-' + t.data]: true}" cx="0" cy="0" class="animate" @click="move(t.tile)" />
                <!-- </transition> -->
              </g>
              <!-- <polyline v-if="winning" :points="line(winning)" /> -->
              <g-poly-line v-if="winning" :tiles="winning" :center="center" :delay="400" />
            </g>
          </svg>
        </div>
      </div>
    ...
    <v-snackbar v-model="snackbar" :timeout="12000" color="grey darken-3" top>
      <v-flex v-touch="{left: () => snackbar = false, right: () => snackbar = false}" @click.native="snackbar = false">
