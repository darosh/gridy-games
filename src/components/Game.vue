<template>
  <div v-resize="onResize" :class="theme">
    ...
    <g-verdict v-model="showVerdict" :reset="reset" :game="game" :theme="theme" />
    ...
    <v-dialog :value="rules" max-width="280px">
      <v-card>
        ...
        <v-card-text class="pt-0 pb-4">
          <div v-for="(r, k) in rulesText" :key="k" class="px-2">{{ r }}</div>
        </v-card-text>
        <div class="text-xs-center">
          <g-board :game="sample" :frame="[240,164]" :margin="4" class="preview d-inline-block" />
        </div>
        <v-card-actions>
          <v-spacer/>
          <v-btn flat style="min-width: 0" @click="rules = false">Ok</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog :value="passConfirm" max-width="280px" persistent>
      <v-card>
        <v-card-title>
          <span class="title pa-2">No moves available</span>
        </v-card-title>
        <div :class="{[theme]: true}" class="text-xs-center">
          <div :class="{['player-' + other]: true}" class="d-inline-block relative player" style="height: 120px; width: 120px">
            <svg height="120" width="120" class="d-block absolute">
              <circle :class="'symbol-' + other" cx="60" cy="60" r="60" />
            </svg>
        ...
        <v-card-actions>
          <v-spacer/>
          <v-btn flat style="min-width: 0" @click="game.start(); passConfirm = false">Pass</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
